...
// Number Multiplier Functions NOW PROJECT 1 FUNCTIONS
function updateFormula() {
    // please write the body of this function
        const operation=document.getElementById("conversion-type").value;
        console.log(operation); // logging selected value
 // logging to the Console to see what the value stored in the variable operation is

        const selectionDisplay=document.getElementById("formula");

        if(operation === 'ftoc') {
            selectionDisplay.innerHTML="°C = (°F - 32) * 5/9"
        } else if (operation === 'ctof') {
            selectionDisplay.innerHTML="°F = (°C x 9/5) + 32" 
        }
...
// Task List Functions TO NOW PROJECT 2 FUNCTIONS
let tasks = [];
...
function validateDate(dateStr) {
  ...
  if (isNaN(monthStr) || isNaN(dayStr)) return false;
...
function validateTime(timeStr) {
  ...
  if (isNaN(hourStr) || isNaN(minStr)) return false;
...
// Calculator Functions NOW PROJECT 3 FUNCTIONS
let memory = 0;
let currentInput = "0";
...
function insertMathFunction(func) {
    ...
    try {
        ...
        if (isNaN(input)) throw "Invalid Input";
        ...
        switch (func) {
            ...
            case 'pow':
                ...
                if (exponent === null) return; // cancel
                result = Math.pow(input, parseFloat(exponent));
                history.innerText = `${input}^${exponent}`;
                break;
            ...
            case 'asin': result = Math.asin(input); break;
            case 'acos': result = Math.acos(input); break;
            case 'atan': result = Math.atan(input); break;
            case 'log':
                if (input <= 0) throw "Math Error";
                result = Math.log10(input); break;
        ...
        if (!['pow'].includes(func)) history.innerText = `${func}(${input})`;
